...
// chooseQRcodeVersion choose the most suitble QR code version for a stated data length in bits,
// the error recovery level required, and the data encoder used.
//
// then choose QR code version is the smallest version for fit numDataBits
// and the optional terminator bits reuqired by the specfied encoder.
//
...
func chooseQRCodeVersion(level RecoveryLevel, encoder *dataEncoder, numDataBits int) *qrCodeVersion {
	var chooseVersion *qrCodeVersion
...
// numTerminatorBitsRequired returns
...
// numBitsToPadToCodeword returns the number if bits required to pad data
// of length numDataBits upto the nearest codeword size.
...
// (which is both the width and height, since QR code are square).
// The Qr code has size symbolSize() x symbolSizr() pixels.
// This does not include the quiet zone.
func (v qrCodeVersion) symbolSize() int {
	return 21 + (v.version-1)*4
}

// quietZoneSize returns the number of pixels of border space on each side
// of the QR code. the quiet space assists with decoding.
